...
`id` sets the HTML `id` attribute on the root element. It is also passed as the second argument to any `title` or `subtitle` function in `ZoneConfig`, so you can brand header text per instance.

<WidgemoShowcase
  description="id=team-widget — the header title function receives it as its second argument."
  data={teamData}
  config={{
    zones: {
      header: {
