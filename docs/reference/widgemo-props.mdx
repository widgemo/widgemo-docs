...
Standard HTML attribute forwarding onto the root element.
...
<Widgemo
  ...
  config={config}
  id="team-table"
  className="my-widget shadow-lg"
/>
```

`className` appends to Widgemo's own root class. `id` sets the HTML `id` attribute — useful for scroll targets, accessibility labels, and CSS selectors.
