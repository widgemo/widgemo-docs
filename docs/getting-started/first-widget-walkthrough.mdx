...
Widgemo measures the nearest zone width via `ResizeObserver`, not the viewport. An action without an `icon` declared will be demoted to the overflow menu whenever the zone is narrower than 1024 px — which includes most docs page widgets and many embedded use cases. **Always provide an `icon` on pinned actions** if they must stay visible at all widths.
